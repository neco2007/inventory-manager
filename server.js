...
// NGワードリスト
const ngWords = ["Copic", "IL BISONTE", "Lindt", "'47", "★wy★", "101 DALMATIANS", "10Gtek", "17906697543", 
...
// aiueo.csv を読み込み、CSVデータを返すエンドポイント
app.get('/products', (req, res) => {
  // aiueo.csv ファイルを同期的に読み込む
  fs.readFile(path.join(__dirname, 'public', 'aiueo.csv'), 'utf8', (err, data) => {
    if (err) {
      res.status(500).send('Error reading CSV file');
      return;
    }

    // PapaParse を使って CSV データをパース
    Papa.parse(data, {
      header: true,         // ヘッダーをデータとして使用
      skipEmptyLines: true, // 空行をスキップ
      complete: (results) => {
        // NGワードを含む商品を除外するフィルタリング
        const filteredProducts = results.data.filter(product => {
          // 商品名にNGワードが含まれていない場合のみ保持
          return !ngWords.some(ngWord => product['商品名'] && product['商品名'].includes(ngWord));
        });

        res.json(filteredProducts); // フィルタリング後の商品データを返す
      }
    });
  });
});
